...
/* 
SK.
crnach model for using the Stuart Morgan varient of the CRN model
in the 2013 Cardioversion study.
...
RHS_CREATE_HEAD(crnach) {
  ...
  #define _(name,initial) (*u)[crnach_##name]=initial;
  #include "crnach_var.h"
  #undef _
} RHS_CREATE_TAIL(crnach,N)
...
RHS_HEAD(crnach,N) {
  ...
  #define _(name,initial) real name=u[crnach_##name];
  #include "crnach_var.h"
  #undef _
  ...
  #define _(name,initial) u[crnach_##name]=name; du[crnach_##name]=0;
  #include "crnach_var.h"
  #undef _
} RHS_TAIL(crnach)
