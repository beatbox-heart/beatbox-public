...
// the pow macros for ints

#define powi_2(x) x*x
#define powi_3(x) x*x*x
#define powi_4(x) x*x*x*x
#define powi_8(x) x*x*x*x*x*x*x*x

// the pow macro for reals

#define powff(x,y) ((x)<(0.0)?(0.0):pow((x),(y)))
   ...
   km2n = Y[8]*1.0;
//   anca = 1.0/(k2n/km2n+pow(1.0+Kmn/Y[33], 4.0)); // 1
   anca = 1.0/(k2n/km2n+ powi_4(1.0+Kmn/Y[33])); // 1
...
//   KsCa = 1.0+0.6/(1.0+pow(3.8e-5/Y[30], 1.4)); // 2
   KsCa = 1.0+0.6/(1.0+ powff(3.8e-5/Y[30], 1.4)); // 2
printf("%f %f\n",Y[30],KsCa);
...
//   INa = GNa*(Y[38]-ENa)*pow(Y[23], 3.0)*((1.0-fINap)*h*Y[21]+fINap*hp*Y[22]); // 3
   INa = GNa*(Y[38]-ENa)*powi_3(Y[23])*((1.0-fINap)*h*Y[21]+fINap*hp*Y[22]); // 3
...
//   allo_i = 1.0/(1.0+pow(KmCaAct/Y[30], 2.0)); // 4
   allo_i = 1.0/(1.0+powi_2(KmCaAct/Y[30])); // 4
...
//  allo_ss = 1.0/(1.0+pow(KmCaAct/Y[33], 2.0)); // 5
  allo_ss = 1.0/(1.0+powi_2(KmCaAct/Y[33])); // 5
...
//   a1 = k1p*pow(Y[36]/Knai, 3.0)/(pow(1.0+Y[36]/Knai, 3.0)+pow(1.0+Y[34]/Kki, 2.0)-1.0); // 6
   a1 = k1p*powi_3(Y[36]/Knai)/(powi_3(1.0+Y[36]/Knai)+powi_2(1.0+Y[34]/Kki)-1.0); // 6

//   b2 = k2m*pow(nao/Knao, 3.0)/(pow(1.0+nao/Knao, 3.0)+pow(1.0+ko/Kko, 2.0)-1.0); // 7
   b2 = k2m*powi_3(nao/Knao)/(powi_3(1.0+nao/Knao)+powi_2(1.0+ko/Kko)-1.0); // 7

//   a3 = k3p*pow(ko/Kko, 2.0)/(pow(1.0+nao/Knao, 3.0)+pow(1.0+ko/Kko, 2.0)-1.0); // 8
   a3 = k3p*powi_2(ko/Kko)/(powi_3(1.0+nao/Knao)+powi_2(1.0+ko/Kko)-1.0); // 8

   b3 = k3m*P*H/(1.0+MgATP/Kmgatp);
//   b4 = k4m*pow(Y[34]/Kki, 2.0)/(pow(1.0+Y[36]/Knai, 3.0)+pow(1.0+Y[34]/Kki, 2.0)-1.0); // 9
  b4 = k4m*powi_2(Y[34]/Kki)/(powi_3(1.0+Y[36]/Knai)+powi_2(1.0+Y[34]/Kki)-1.0); // 9
  ...
  //    Istim = 0.0; // deal with this in a bit.

   dY[34] = -(Ito+IKr+IKs+IK1+IKb/*+Istim*/-2.0*INaK)*cm*Acap/(F*vmyo)+JdiffK*vss/vmyo;
   dY[35] = -ICaK*cm*Acap/(F*vss)-JdiffK;
//   Bcai = 1.0/(1.0+cmdnmax*kmcmdn/pow(kmcmdn+Y[30], 2.0)+trpnmax*kmtrpn/pow(kmtrpn+Y[30], 2.0)); // 10
   Bcai = 1.0/(1.0+cmdnmax*kmcmdn/powi_2(kmcmdn+Y[30])+trpnmax*kmtrpn/powi_2(kmtrpn+Y[30])); // 10


   dY[30] = Bcai*(-(IpCa+ICab-2.0*INaCa_i)*cm*Acap/(2.0*F*vmyo)-Jup*vnsr/vmyo+Jdiff*vss/vmyo);
//   Bcass = 1.0/(1.0+BSRmax*KmBSR/pow(KmBSR+Y[33], 2.0)+BSLmax*KmBSL/pow(KmBSL+Y[33], 2.0)); // 11
   Bcass = 1.0/(1.0+BSRmax*KmBSR/powi_2(KmBSR+Y[33])+BSLmax*KmBSL/powi_2(KmBSL+Y[33])); // 11
...
//   Bcajsr = 1.0/(1.0+csqnmax*kmcsqn/pow(kmcsqn+Y[31], 2.0)); // 12
   Bcajsr = 1.0/(1.0+csqnmax*kmcsqn/powi_2(kmcsqn+Y[31])); // 12
   dY[31] = Bcajsr*(Jtr-Jrel);
   dY[38] = -(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa_i+INaCa_ss+INaK+INab+IKb+IpCa+ICab/*+Istim*/);
//   Jrel_inf_temp = a_rel*-ICaL/(1.0+1.0*pow(1.5/Y[31], 8.0)); // 13
  Jrel_inf_temp = a_rel*-ICaL/(1.0+1.0*powi_8(1.5/Y[31])); // 13
   ...
   Jrel_temp = a_relp*-ICaL/(1.0+pow(1.5/Y[31], 8.0)); // 14
//   Jrel_temp = a_relp*-ICaL/(1.0+powi_8(1.5/Y[31])); // 14
