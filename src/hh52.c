...
/* sed 's/(hh52/(hh52m/g;s/#define GATE 1/#define GATE 2/g;' hh52.c > hh52m.c */
...
CHANNEL_TR_MATRIX(hh52_ik){	/* described above */
  #define V (u[0])
  /* transition rates */
...
IONIC_CREATE_HEAD (hh52)
{
  ...
  */
  ...
  sbch = &(ch->subchain[0]);
  ch->dimension = NM_ina;
  SUBCHAIN(hh52_ina, -1, -200, 200, 0.01, 0);
  /* intialize IK Markov chains */
  ch += 1;
  sbch = &(ch->subchain[0]);
  ch->dimension = NM_ik;
  SUBCHAIN(hh52_ik, -1, -200, 200, 0.01, 0);  
