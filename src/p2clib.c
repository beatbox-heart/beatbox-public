...
/* now functions added by VNB */
/* Choose Unix-compatible functions */

void  GetTime(
  unsigned short *hour,
  unsigned short *minute,
  unsigned short *second,
  unsigned short *sec100
) {
  time_t ti = time(NULL);
...
/* DANGEROUS !! In source, place erase everywhere BEFORE =NULL !! */
static char FILE_NAME[FOPEN_MAX][MAXPATH];
void assign(FILE *f, char *name) {
  assert(f != NULL);
  assert(fileno(f)<FOPEN_MAX);
  strncpy(FILE_NAME[fileno(f)],name,MAXPATH);
}
void erase(FILE *f) {
  assert(f != NULL);
  unlink(FILE_NAME[fileno(f)]);
}
...
unsigned long DiskFree(unsigned char drive) {
  struct dfree df;
  getdfree(drive,&df);
  return ( (long)df.df_avail*(long)df.df_bsec*(long)df.df_sclus );
}
